...
function WRBar({win,lose,highlight}) {
  ...
  if(num===null) return <span style={{color:"#505880",fontSize:13}}>—</span>;
  ...
  return (
    <div style={{display:"flex",alignItems:"center",gap:8,flex:1}}>
      ...
      <span style={{fontSize:10,color:"#c0cce0",minWidth:72}}>{win}W / {lose}L</span>
    </div>
  );
}
...
function StatCard({label,value,sub,color="#00ff9d",small}) {
  return (
    <div style={{background:"#111128",border:"1px solid #1e2040",padding:"12px 14px",borderRadius:2}}>
      <div style={{fontSize:9,letterSpacing:3,color:"#c0cce0",marginBottom:6,textTransform:"uppercase"}}>{label}</div>
      <div style={{fontSize:small?12:18,fontWeight:"bold",color,lineHeight:1.1}}>{value}</div>
      {sub&&<div style={{fontSize:10,color:"#b0bcd0",marginTop:4}}>{sub}</div>}
    </div>
  );
}
...
function NightAlert({nightNum,dayNum}) {
  ...
  if(edge>=3) return <div style={{background:"#001f10",border:"1px solid #00ff9d",padding:"6px 12px",fontSize:10,color:"#00ff9d",letterSpacing:1,marginBottom:12,borderRadius:2}}>★ NIGHT EDGE ACTIVE — Night outperforming day by {edge.toFixed(1)}pts</div>;
  if(edge<-2) return <div style={{background:"#200008",border:"1px solid #ff4d6d",padding:"6px 12px",fontSize:10,color:"#ff4d6d",letterSpacing:1,marginBottom:12,borderRadius:2}}>⚠ NIGHT EDGE OFFLINE — Day outperforming night by {Math.abs(edge).toFixed(1)}pts</div>;
  return null;
}
...
export default function App() {
  ...
  // ── Styles ────────────────────────────────────────────────────────────────────
  const S={
    inp:{background:"#0a0a1a",border:"1px solid #1e2040",color:"#ffffff",fontFamily:"'Courier New',monospace",fontSize:11,padding:"5px 8px",outline:"none",borderRadius:1},
    panel:{background:"#0d0d1f",border:"1px solid #1e2040",padding:"14px 16px",marginBottom:12,borderRadius:2},
    secT:{fontSize:9,letterSpacing:3,color:"#c0cce0",textTransform:"uppercase",marginBottom:10,borderBottom:"1px solid #1e2040",paddingBottom:5},
    ...
    lbl:(hl)=>({fontSize:11,color:hl?"#00ff9d":"#ffffff",minWidth:88,fontWeight:hl?"bold":"normal"}),
    btn:(v,d)=>({background:d?"#0d0d1f":v==="danger"?"transparent":v==="sec"?"#151530":"#001f10",border:`1px solid ${d?"#252845":v==="danger"?"#ff4d6d":v==="sec"?"#303060":"#00ff9d"}`,color:d?"#404468":v==="danger"?"#ff4d6d":v==="sec"?"#9090c0":"#00ff9d",fontFamily:"'Courier New',monospace",fontSize:10,letterSpacing:2,padding:"7px 16px",cursor:d?"not-allowed":"pointer",borderRadius:3,transition:"all 0.15s"}),
    mainTab:(a)=>({background:"none",border:"none",color:a?"#00ff9d":"#a0b0c8",fontFamily:"'Courier New',monospace",fontSize:11,letterSpacing:2,padding:"12px 20px",cursor:"pointer",borderBottom:a?"2px solid #00ff9d":"2px solid transparent",textTransform:"uppercase",transition:"color 0.15s"}),
    subTab:(a)=>({background:"none",border:"none",color:a?"#00ff9d":"#a0b0c8",fontFamily:"'Courier New',monospace",fontSize:9,letterSpacing:2,padding:"8px 16px",cursor:"pointer",borderBottom:a?"2px solid #00ff9d":"2px solid transparent",textTransform:"uppercase"}),
    seg:(a)=>({background:a?"#001f10":"#0d0d1f",border:`1px solid ${a?"#00ff9d":"#252845"}`,color:a?"#00ff9d":"#b0bcd0",fontFamily:"'Courier New',monospace",fontSize:9,letterSpacing:1,padding:"6px 12px",cursor:"pointer",borderRadius:3,transition:"all 0.15s"}),
  };
  ...
  return (
    <div style={{fontFamily:"'Courier New',monospace",background:"#080818",minHeight:"100vh",color:"#ffffff"}}>
      ...
      <div style={{background:"#0d0d1f",borderBottom:"1px solid #1e2040",padding:"12px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div>
          ...
          <div style={{fontSize:9,color:"#7080a0",letterSpacing:2,marginTop:2,color:"#8090b0"}}>COPY-TRADE INTELLIGENCE DASHBOARD</div>
        </div>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:3}}>
          {walletLabel&&<div style={{fontSize:10,color:"#b0bcd0"}}>{walletLabel} · {trades.length} trades</div>}
      ...
      {fetchStatus!=="idle"&&(
        <div style={{background:"#0d0d1f",borderBottom:"1px solid #1e2040",padding:"6px 20px",display:"flex",alignItems:"center",gap:10}}>
          ...
          {fetchStatus!=="loading"&&<button onClick={()=>setFetchStatus("idle")} style={{background:"none",border:"none",color:"#7080a0",cursor:"pointer",fontSize:11,marginLeft:"auto"}}>✕</button>}
        </div>
      )}
      ...
      {storageStatus==="ready"&&trades.length>0&&fetchStatus==="idle"&&lastSaved&&(
        <div style={{background:"#001510",borderBottom:"1px solid #005528",padding:"5px 20px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <span style={{fontSize:10,color:"#00cc70",letterSpacing:1}}>✓ {trades.length} TRADES RESTORED FROM STORAGE · Last saved {lastSaved}</span>
          <button onClick={()=>{if(window.confirm("Clear all stored trades?"))clearStorage();}} style={{background:"none",border:"none",color:"#7080a0",cursor:"pointer",fontSize:10,letterSpacing:1}}>CLEAR STORAGE</button>
        </div>
      )}
              ...
              <div>
                <div style={S.panel}>
                  ...
                  <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
                    {[["DATE","date",manualDate,e=>setManualDate(e.target.value)],["COUNT","number",manualCount,e=>setManualCount(Math.max(1,parseInt(e.target.value)||1))]].map(([lbl,type,val,fn])=>(<div key={lbl} style={{display:"flex",flexDirection:"column",gap:3}}><label style={{fontSize:9,letterSpacing:2,color:"#7080a0"}}>{lbl}</label><input type={type} value={val} onChange={fn} style={{...S.inp,width:type==="number"?55:undefined}}/></div>))}
                    <div style={{display:"flex",flexDirection:"column",gap:3}}><label style={{fontSize:9,letterSpacing:2,color:"#7080a0"}}>HOUR (ET)</label><select value={manualHour} onChange={e=>setManualHour(parseInt(e.target.value))} style={S.inp}>{HOURS.map(h=><option key={h} value={h}>{HOUR_LABELS[h]} ({String(h).padStart(2,"0")}:00)</option>)}</select></div>
                    <div style={{display:"flex",flexDirection:"column",gap:3}}><label style={{fontSize:9,letterSpacing:2,color:"#7080a0"}}>RESULT</label><select value={manualResult} onChange={e=>setManualResult(e.target.value)} style={S.inp}><option value="win">WIN</option><option value="loss">LOSS</option></select></div>
                ...
                <div style={S.panel}>
                  ...
                  <div style={{fontSize:10,color:"#9090b0",marginBottom:6}}>Format: <span style={{color:"#ffffff"}}>YYYY-MM-DD, HOUR, win/loss[, avgPrice, size]</span></div>
                  <textarea value={bulkText} onChange={e=>setBulkText(e.target.value)} placeholder={"2026-03-03, 3, win, 0.6500, 0.0154\n2026-03-03, 4, loss, 0.5200, 0.0200"} style={{width:"100%",background:"#080818",border:"1px solid #1e2040",color:"#ffffff",fontFamily:"'Courier New',monospace",fontSize:11,padding:9,resize:"vertical",minHeight:90,outline:"none",boxSizing:"border-box",borderRadius:1}}/>
                ...
                <div style={{fontSize:10,color:"#b0bcd0"}}>{trades.length} trades loaded · Storage: <span style={{color:storageColors[storageStatus]}}>{storageStatus}</span></div>
              </div>
            )}
            {wrSubTab==="history"&&(
              <div style={S.panel}>
                <div style={S.secT}>TRADE HISTORY ({trades.length})</div>
                {trades.length===0&&<div style={{color:"#505880",fontSize:12,padding:"16px 0"}}>No trades loaded.</div>}
                <div style={{maxHeight:420,overflowY:"auto"}}>
                  {[...trades].sort((a,b)=>b.dateET.localeCompare(a.dateET)||b.hourET-a.hourET).map(t=>(
                    <div key={t.id} style={{display:"flex",gap:12,padding:"4px 0",borderBottom:"1px solid #131330",fontSize:11,alignItems:"center"}}>
                      ...
                      <span style={{color:"#505880",fontSize:10}}>{t.avgPrice>0?`@${t.avgPrice.toFixed(3)}`:""}</span>
                      <button onClick={()=>setTrades(tr=>tr.filter(x=>x.id!==t.id))} style={{color:"#505880",background:"none",border:"none",cursor:"pointer",fontSize:10,marginLeft:"auto"}}>✕</button>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {mainTab==="pnl"&&(
        <div style={{padding:"16px 20px"}}>
          <div style={S.panel}>
            ...
            <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
              <div style={{display:"flex",flexDirection:"column",gap:3,flex:1,minWidth:180}}><label style={{fontSize:9,letterSpacing:2,color:"#7080a0"}}>MARKET KEYWORD</label><input value={pnlFilter} onChange={e=>setPnlFilter(e.target.value)} style={{...S.inp,width:"100%"}} placeholder="e.g. Bitcoin Up or Down"/></div>
              <div style={{display:"flex",flexDirection:"column",gap:3}}><label style={{fontSize:9,letterSpacing:2,color:"#7080a0"}}>LOOKBACK</label><select value={pnlHours} onChange={e=>setPnlHours(parseInt(e.target.value))} style={S.inp}>{[24,48,72,168,336,720].map(h=><option key={h} value={h}>{h}h ({Math.round(h/24)}d)</option>)}</select></div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
            {[{label:"NIGHT PnL (12AM–7AM)",d:pnlStats.night},{label:"DAY PnL (7AM–12AM)",d:pnlStats.day}].map(({label,d})=>{const total=d.w+d.l,wr=total>0?(100*d.w/total).toFixed(1):null,col=d.pnl>0?"#00ff9d":d.pnl<0?"#ff4d6d":"#7080a0";return(<div key={label} style={{background:"#0d0d1f",border:"1px solid #1e2040",padding:"14px 16px"}}><div style={{fontSize:9,letterSpacing:2,color:"#c0cce0",marginBottom:6}}>{label}</div><div style={{fontSize:24,fontWeight:"bold",color:col}}>{d.pnl>=0?"+":""}${d.pnl.toFixed(4)}</div><div style={{fontSize:10,color:"#b0bcd0",marginTop:4}}>{total>0?`${d.w}W / ${d.l}L · ${wr}% WR`:"No data"}</div></div>);})}
          </div>
          <div style={S.panel}>
            <div style={S.secT}>PnL BY 4HR BLOCK</div>
            {BLOCK4_LABELS.map((label,b)=>{const d=pnlStats.byBlock[b],col=d.pnl>0?"#00ff9d":d.pnl<0?"#ff4d6d":"#7080a0",wr=d.w+d.l>0?(100*d.w/(d.w+d.l)).toFixed(1):null;return(<div key={b} style={{...S.row(false),padding:"6px 0"}}><div style={{...S.lbl(false),minWidth:100}}>{label}</div><span style={{color:col,fontWeight:"bold",minWidth:90,fontSize:12}}>{d.pnl>=0?"+":""}${d.pnl.toFixed(4)}</span><span style={{fontSize:10,color:"#b0bcd0"}}>{d.count>0?`${d.w}W/${d.l}L ${wr}% · ${d.count} trades`:"—"}</span></div>);})}
          </div>
          {openPos.length>0&&(<div style={S.panel}><div style={S.secT}>OPEN POSITIONS ({openPos.length})</div>{openPos.map((p,i)=>{const size=parseFloat(p.size||0),avg=parseFloat(p.avgPrice||0),cur=parseFloat(p.curPrice||0),unreal=size>0&&avg>0&&cur>0?size*(cur-avg):null,col=unreal===null?"#7080a0":unreal>=0?"#00ff9d":"#ff4d6d";return(<div key={i} style={{padding:"7px 0",borderBottom:"1px solid #131330"}}><div style={{fontSize:11,color:"#ffffff",marginBottom:3}}>{(p.title||"").slice(0,62)}{(p.title||"").length>62?"…":""}</div><div style={{fontSize:10,color:"#ffffff",display:"flex",gap:16,flexWrap:"wrap"}}><span>{size.toFixed(2)} {p.outcome}</span><span>avg {avg.toFixed(3)}</span><span>cur {cur.toFixed(3)}</span>{unreal!==null&&<span style={{color:col,fontWeight:"bold"}}>{unreal>=0?"+":""}${unreal.toFixed(4)}</span>}</div></div>);})}</div>)}
          {pnlData.length===0&&pnlStatus!=="loading"&&(<div style={{textAlign:"center",padding:"40px 0",color:"#505880",fontSize:11,letterSpacing:2}}>SET FILTERS ABOVE AND CLICK FETCH PnL</div>)}
        </div>
      )}
      ...
      {mainTab==="bt"&&(
        <div style={{padding:"16px 20px"}}>
          {trades.length===0?(
            <div style={{textAlign:"center",padding:"40px 0",color:"#505880",fontSize:11,letterSpacing:2}}>FETCH WALLET DATA FIRST (FETCH BUTTON IN HEADER)</div>
          ):(
            <div>
              <div style={S.panel}>
                ...
                <div style={{display:"flex",gap:16,flexWrap:"wrap",alignItems:"flex-start"}}>
                  <div style={{display:"flex",flexDirection:"column",gap:3}}>
                    <label style={{fontSize:9,letterSpacing:2,color:"#7080a0"}}>STARTING BALANCE ($)</label>
                    <input type="number" value={btStartBal} onChange={e=>setBtStartBal(e.target.value)} style={{...S.inp,width:90}}/>
                  </div>
                  <div style={{display:"flex",flexDirection:"column",gap:3}}>
                    <label style={{fontSize:9,letterSpacing:2,color:"#7080a0"}}>POSITION SIZING</label>
                    ...
                    {btSizingMode==="fixed"&&<div style={{display:"flex",alignItems:"center",gap:6,marginTop:4}}><span style={{fontSize:10,color:"#b0bcd0"}}>$</span><input type="number" value={btFixedAmt} onChange={e=>setBtFixedAmt(e.target.value)} style={{...S.inp,width:70}}/><span style={{fontSize:10,color:"#b0bcd0"}}>per trade</span></div>}
                    {btSizingMode==="pct"&&<div style={{display:"flex",alignItems:"center",gap:6,marginTop:4}}><input type="number" value={btPct} onChange={e=>setBtPct(e.target.value)} style={{...S.inp,width:55}}/><span style={{fontSize:10,color:"#b0bcd0"}}>% of balance per trade</span></div>}
                    {btSizingMode==="wallet"&&<div style={{fontSize:10,color:"#b0bcd0",marginTop:4}}>Uses wallet's actual contract sizes</div>}
                  </div>
                  <div style={{display:"flex",flexDirection:"column",gap:3,flex:1}}>
                    <label style={{fontSize:9,letterSpacing:2,color:"#7080a0"}}>TIME BLOCK MODE</label>
                    ...
                    {btMode==="custom"&&(
                      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:4,alignItems:"center"}}>
                        ...
                        <span style={{fontSize:10,color:"#b0bcd0"}}>to</span>
                ...
                <div style={S.panel}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:8}}>
                    {allBlocksResults.map(({label,result,tradeCount})=>{
                      if(!result||tradeCount===0) return(<div key={label} style={{background:"#111128",border:"1px solid #1e2040",padding:"12px",borderRadius:2}}><div style={{fontSize:9,letterSpacing:2,color:"#c0cce0",marginBottom:6}}>{label}</div><div style={{fontSize:12,color:"#505880"}}>No trades</div></div>);
                      ...
                      return(
                        <div key={label} style={{background:"#111128",border:"1px solid #1e2040",padding:"12px",borderRadius:2}}>
                          <div style={{fontSize:9,letterSpacing:2,color:"#7080a0",marginBottom:4}}>{label}</div>
                          <div style={{fontSize:20,fontWeight:"bold",color:roiCol}}>{result.roi>=0?"+":""}{result.roi.toFixed(1)}%</div>
                          <div style={{fontSize:10,color:"#ffffff",marginTop:3}}>{fmtMoney(result.endBal-result.startBal)} · {tradeCount}t</div>
                          <div style={{fontSize:10,color:wrColor(pctNum(result.wins,result.losses)),marginTop:2}}>{pct(result.wins,result.losses)}% WR</div>
                          <div style={{fontSize:10,color:"#b0bcd0"}}>DD {result.maxDD.toFixed(1)}%</div>
                        </div>
                      );
                    })}
                  </div>
              ...
              {btMode!=="all"&&btResult&&(
                <div>
                  ...
                  <div style={S.panel}>
                    ...
                    {btResult.equity.length>1?(
                      <ResponsiveContainer width="100%" height={200}>
                        <LineChart data={btResult.equity} margin={{top:5,right:10,left:0,bottom:5}}>
                          ...
                          <YAxis domain={["auto","auto"]} tick={{fill:"#7080a0",fontSize:10}} width={55} tickFormatter={v=>`$${v}`}/>
                          <Tooltip contentStyle={{background:"#0d0d1f",border:"1px solid #1e2040",fontFamily:"'Courier New',monospace",fontSize:10}} formatter={(v)=>[`$${v.toFixed(2)}`,"Balance"]} labelFormatter={i=>`Trade ${i}`}/>
                    ...
                    ):<div style={{color:"#505880",fontSize:12,padding:"20px 0",textAlign:"center"}}>Not enough data</div>}
                  </div>
                </div>
              )}
              {btMode!=="all"&&btFilteredTrades.length===0&&(<div style={{textAlign:"center",padding:"30px 0",color:"#505880",fontSize:11,letterSpacing:2}}>NO TRADES MATCH SELECTED BLOCK / DATE RANGE</div>)}
            </div>
          )}
        </div>
      )}
